...
# ============ 1. 표지 ============
s = new_slide()
runs(s, 0.75, 0.55, 6.0, 0.35, [[("네이버클라우드 AX Lab", True, GREY, 12)]])
...
items = [
    ...
    ("질문을 “누가 이기나”에서 “언제 기여하나”로 바꾸면 답이 있습니다", GREEN,
     [("모형 간 우열 대신 조건별 기여를 보면 — 정보가 얇은 조기 전망주차와 경기 반등기에서 사전학습", False),
...
# ============ 4. 전망시계 진단 ============
s = new_slide()
header(s, "3  |  전망시계 진단", "정보가 얇은 조기 구간에서는 XGBoost의 우위가 사라집니다")
...
runs(s, X2, 1.95, W, 0.35, [[("직접학습 신경망 이식판의 실측 (참고)", True, NAVY, 12.5)]])
hline(s, X2, 2.32, W, LINE, 1.0)
runs(s, X2, 2.48, W, 3.2, [
    [("사내 어텐션 LSTM 계열(BISTRO 유사 구조)을 당사가 GDP 과제용", False, INK, 10.5)],
    [("으로 재구현해 동일 규약으로 채점했습니다.", False, INK, 10.5)],
    [("", False, INK, 5)],
...
runs(s, X2 + 0.2, 5.7, W - 0.4, 0.8, [
    [("유의: 본 결과는 당사 재구현 이식판에 대한 동일 규약 판정이며, 원 연구나", False, GREY, 9.3)],
    [("타 과제(물가 등) 성능에 대한 판정이 아닙니다. 원본도 동일 규약으로 채점", False, GREY, 9.3)],
    [("하면 공정 비교가 가능합니다.", False, GREY, 9.3)],
], sp=1.5)
footnote(s, RULE)
...
runs(s, X2 + 0.25, 1.78, W2 - 0.45, 4.6, [
    ...
    [("", False, INK, 5)],
    [("정보 빈곤 구간. ", True, INK, 10.5), ("발표 19~14주 전 —", False, INK, 10.5)],
    [("공식지표 공백기를 세계 지식과 일별 신호로", False, INK, 10.5)],
    [("보완합니다(3장).", False, INK, 10.5)],
    [("", False, INK, 5)],
...
rows = [
    ...
    ("트랜스포머류", "점 예측 대체재가 아니라 정보 빈곤 구간·반등기의 조건부 보완재이며, 불확실성 정량화의 재료입니다.", "논문 핵심 재료"),
