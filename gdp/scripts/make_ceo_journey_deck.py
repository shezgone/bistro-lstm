...
phases = [
    ("6월", "착수·재현", "한은 현행 시스템(DFM+XGBoost)을 실시간 빈티지로 재현 — 모든 비교의 잣대 확립", BLUE),
...
for when, title, desc, c in phases:
    ...
    y += 1.06
footnote(s, RULE)
notes(s, "여정의 구조가 메시지: 우리는 실패를 숨기지 않고 기록했고, 그 기록(무엇이 왜 안 되는가)이 지금 한은과의 공동 논문 재료가 됨. 8월 기준선 격변 때 당일 재검증으로 대응한 것이 신뢰의 전환점.")
...
# ============ 10. 성과와 다음 ============
s = new_slide()
header(s, "9  |  성과와 다음 단계", "협업의 위치가 바뀌었습니다 — 검증 파트너에서 공동 연구 파트너로")
items = [
...
for t, d, c in items:
    ...
    y += 1.3
runs(s, 0.6, 6.75, 12.0, 0.3, [[("FDE로서의 3개월: 고객의 코드 결함에 당일 재검증으로, 고객의 진단에 원인 규명으로, 고객의 제안에 첫 실증으로 답했습니다.", True, NAVY, 11)]])
footnote(s, RULE)
