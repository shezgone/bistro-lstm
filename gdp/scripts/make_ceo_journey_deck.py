...
runs(s, 0.78, 6.25, 11.2, 0.35, [[("한은–네이버 2026년 협력과제 8건 중 우선순위 2위 「시계열 Track1 (거시경제 예측)」 — 한은 담당: 이창훈·윤희근 (디지털혁신실)", False, GREY, 10.5)]])
runs(s, 0.78, 6.6, 8.0, 0.35, [[("2026. 9.  |  대표 보고  |  작성: 김용민", False, GREY, 11)]])
notes(s, "3개월 여정의 한 줄 요약: 정면 승부(국면전환) → 통계적 벽 확인 → 축 전환(조건부 가치) → 고객이 협업 확대를 제안하는 위치까지. 발표 시간 10분 기준, 2·5·9번 장이 핵심. (참고: 대표님은 한은 FDE 전사 사례 보고 — 1순위 규정 온톨로지, 코딩 어시스턴트 등 — 를 이미 받으심. 본 덱은 그 지도에서 우선순위 2위 '시계열 Track1'의 상세 보고에 해당. 시계열 Track2(뉴스 인과관계, 데이터사이언스팀 이현규 과장)와는 별개 트랙.")

# ============ 2. 과제와 여정 지도 ============
s = new_slide()
header(s, "1  |  과제와 여정", "요약 — 정확도 경쟁에서 출발해, 평가 기준을 함께 다시 세우는 데까지 왔습니다")
phases = [
    ("~5월 (전사)", "CPI 연구", "물가(CPI) 예측으로 시작 — 소형 특화 모델·FM·언어모델(HCX·Claude) 3계열 비교 체계 구축. 6월 한은 요청으로 대상을 GDP 성장률로 전환 (검증 하네스는 계승)", GREY),
    ("6월", "착수·재현", "한은 측의 시계열 파운데이션 모델(BISTRO, BIS WP 1337) 해석 질의에서 출발. FDE Consulting & Planning팀(김지혜·김민승)과 초기 방향성을 함께 수립하고, 현행 시스템(DFM+XGBoost)을 실시간 빈티지로 재현", BLUE),
...
for when, title, desc, c in phases:
    sidebar(s, 0.6, y, 0.8, c)
    runs(s, 0.85, y, 1.35, 0.4, [[(when, True, c, 11.5)]])
    runs(s, 2.25, y, 2.2, 0.4, [[(title, True, NAVY, 11.5)]])
    runs(s, 4.55, y, 8.1, 0.75, [[(desc, False, INK, 10)]])
    y += 0.93
footnote(s, RULE)
...
runs(s, 0.6, 1.45, 6.1, 4.4, [
    ...
    [("· GDP는 분기 지표 — 1년에 관측 4개, 8년을 다 모아도 32개.", False, INK, 10.5)],
    [("  저표본에서 패턴을 찾아야 하는 과제의 구조적 어려움", False, INK, 10.5)],
    [("· 주간 19발은 분기 내 강한 상관 —", False, INK, 10.5)],
...
runs(s, 0.6, 1.45, 6.0, 4.5, [
    ...
    [("· 언어모델(Claude·HCX) 시계열 활용도 타진 — 가능성은", False, INK, 10.5)],
    [("  관찰됐으나 통계적 유의성 미확보, 학습데이터 오염으로", False, INK, 10.5)],
    [("  실시간 공정 채점이 성립하지 않아 채택 제외", False, INK, 10.5)],
...
pairs = [
    ("6월~  빈티지 데이터셋·평가 기준 정비", "주간 실시간 빈티지(73개 지표)·기준모형 예측·평가 그리드를 정비해 제공 — 협업의 공통 잣대를 한은이 먼저 명확히 함",
     "그 잣대 위에 하네스", "한은 제공 빈티지 데이터셋·전처리 코드를 그대로 재사용해 재현 채점 체계 구축 — 이후 모든 실험(양측)이 같은 잣대로 비교 가능해짐"),
...
for lt, ld, rt, rd in pairs:
    ...
    y += 1.06
rect(s, 0.6, 6.54, 12.13, 0.54, BGGREY, None)
sidebar(s, 0.6, 6.54, 0.54, NAVY)
runs(s, 0.85, 6.6, 11.7, 0.46, [
    [("모든 과정이 커밋 기록으로 남아 언제든 재현하고 감사할 수 있습니다 — 실험 코드의 재현성도 서로 확인하며 진행했습니다.", True, NAVY, 10)],
    [("(한은의 개정 수치를 당사가 재계산으로 일치 확인, 당사의 조기 결합 아이디어를 한은이 자체 탐색으로 재현)", False, GREY, 9)]])
...
items = [
    ("고객 조직 지형", "디지털혁신실(협업 파트너)은 딥러닝 기반 나우캐스팅 시스템 개선을, 경제모형실은 전문가 판단 기반 전망을 맡는 구도 — 경제모형실이 최근 산학협력으로 BISTRO 시계열 모델을 연구 중이어서 두 조직은 경쟁 관계로 보임. 당사의 BISTRO 원본 실측·적응 실증은 파트너가 이 지형에서 기술 근거를 갖게 하는 위치", WARM),
...
y = 1.45
for t, d, c in items:
    sidebar(s, 0.6, y, 0.92, c)
    runs(s, 0.85, y, 2.1, 0.4, [[(t, True, NAVY, 12)]])
    runs(s, 3.05, y, 9.6, 0.9, [[(d, False, INK, 10)]])
    y += 1.06
runs(s, 0.6, 6.78, 12.0, 0.3, [[("FDE로서의 3개월 — 출발점이었던 BISTRO 해석 질의에 원본 실측과 적응 개선으로 답했습니다. 코드 개정에는 당일 재검증으로, 진단에는 원인 규명으로, 제안에는 첫 실증으로 대응했습니다.", True, NAVY, 11)]])
footnote(s, RULE)
notes(s, "마지막 문장이 대표 보고의 결론 — FDE 모델의 가치 입증. 다음 결정 3건 중 대표 승인이 필요한 것은 특별히 없으며(진행 보고), 논문 저자 구성 등 회사 차원 조율이 필요해지면 별도 보고 예정. (조직 지형 구두 보충: 경제모형실 산학협력의 학계 파트너는 서범석 교수로 파악 — 문서 명시는 피하고 구두로만. BISTRO 관련 대외 서술은 항상 '동일 규약 실측·공개 체크포인트 기준' 프레임 유지.)")
