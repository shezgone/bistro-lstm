...
# ============ 2. 과제와 여정 지도 ============
s = new_slide()
header(s, "1  |  과제와 여정", "한 문장 요약 — 정면 승부에서 시작해, 게임의 규칙을 다시 정의하는 것으로 끝났습니다")
phases = [
    ("6월", "착수·재현", "한은 측의 시계열 파운데이션 모델(BISTRO, BIS WP 1337) 해석 질의에서 출발 — 현행 시스템(DFM+XGBoost)을 실시간 빈티지로 재현해 검증 기반 확립", BLUE),
...
# ============ 5. 2막: 검정력의 벽 ============
s = new_slide()
header(s, "4  |  2막 · 통계적 벽", "\"이 표본에서는 누구도 우승을 증명할 수 없다\" — 그래서 게임을 바꿨습니다")
runs(s, 0.6, 1.45, 6.1, 4.4, [
...
# ============ 6. 3막: 기준선 격변 ============
s = new_slide()
header(s, "5  |  3막 · 기준선 격변", "고객의 코드 개정에 당일 대응 — 위기가 신뢰의 전환점이 됐습니다")
...
# ============ 7. 4막: FM 전수 검증 ============
s = new_slide()
header(s, "6  |  4막 · 파운데이션 모델 전수 검증", "\"어떤 딥러닝이 되는가\"를 조건으로 답했습니다 — BIS 모델 원본 실측 포함")
...
# ============ 8. 5막: LoRA 적응 ============
s = new_slide()
header(s, "7  |  5막 · 적응학습(LoRA) 실증", "한은이 제안한 방향을 당사가 먼저 실증 — 평탄성이 풀리기 시작했습니다")
runs(s, 0.6, 1.45, 6.0, 4.5, [
...
# ============ 9. 종합 성적 ============
s = new_slide()
header(s, "8  |  종합", "최종 구성 — 기준선 대비 -2.3%, 여정의 모든 발견이 한 구조에 들어있습니다")
...
# ============ 10. 협업 방식 (FDE) ============
s = new_slide()
header(s, "9  |  일하는 방식", "회의보다 커밋 — 모든 협업이 GitHub 문서·코드 리뷰로 진행됐습니다")
runs(s, 0.6, 1.42, 12.1, 0.35, [
    [("한은이 코드와 리포트를 커밋하면, 당사가 커밋 단위로 리뷰·재검증하고 재현 가능한 스크립트와 문서로 회신하는 비동기 협업 — 요구사항 분석이 곧 코드 리뷰였습니다.", False, INK, 10.5)],
])
# 커밋 → 응답 대응표
...
pairs = [
    ("8/4  \"코드 수정 및 산출물 저장\"", "예측단위 개정(schema v2) — diff에서 단위 결함 수정과 검증 로직 강화를 파악",
     "당일 재검증", "40여 구성 전면 재채점 → 신 기준선(XGBoost 단독) 확정, 당사 종전 기록도 함께 폐기"),
    ("8/25  \"Add BISTRO nowcasting experiments...\"", "실험 코드 16종 + 테스트 + 회의자료(typ) 3.7만 줄 — 코드에서 실험 설계·한계까지 독해",
     "익일 진단 재현 → 3일 내 첫 실증", "주차별 흡수력 진단 재현·원인 규명(회의대응 1p) → 그들이 제안만 한 적응학습을 LoRA로 첫 실측"),
...
for lt, ld, rt, rd in pairs:
    sidebar(s, 0.6, y, 1.15, NAVY)
    runs(s, 0.82, y, 5.7, 0.35, [[(lt, True, INK, 10.5)]])
    runs(s, 0.82, y + 0.34, 5.75, 0.8, [[(ld, False, GREY, 9.3)]])
    sidebar(s, 6.9, y, 1.15, GREEN)
    runs(s, 7.12, y, 2.6, 0.35, [[(rt, True, GREEN, 10.5)]])
    runs(s, 7.12, y + 0.34, 5.55, 0.8, [[(rd, False, INK, 9.3)]])
    y += 1.32
rect(s, 0.6, 6.5, 12.13, 0.5, BGGREY, None)
sidebar(s, 0.6, 6.5, 0.5, NAVY)
runs(s, 0.85, 6.58, 11.7, 0.35, [
    [("이 방식의 가치: 협업의 전 과정이 커밋 로그로 남아 재현·감사가 가능 — 중앙은행과 일하는 형식 그 자체가 신뢰의 근거가 됩니다.", True, NAVY, 10.5)]])
footnote(s, "당사 저장소 커밋 90여 건(6~8월) · 한은 저장소는 pull 전용(푸시하지 않음) — 산출물은 문서·PDF로 전달.")
notes(s, "FDE 활동의 실체를 보여주는 장. 핵심 주장: 고객의 커밋을 읽는 능력(diff에서 단위 결함·실험 설계를 독해)과 재현 가능한 코드로 회신하는 속도(당일 재검증, 3일 내 실증)가 FDE의 차별점. 기밀 관리도 언급 가능 — 한은 빈티지 데이터는 저장소 분리·커밋 금지 규칙 유지.")

# ============ 11. 성과와 다음 ============
s = new_slide()
header(s, "10  |  성과와 다음 단계", "협업의 위치가 바뀌었습니다 — 검증 파트너에서 공동 연구 파트너로")
items = [
...
for t, d, c in items:
    ...
    y += 1.3
runs(s, 0.6, 6.75, 12.0, 0.3, [[("FDE로서의 3개월: 출발점이었던 BISTRO 해석 질의에 원본 실측과 적응 개선으로 답하며 한 바퀴를 완성 — 고객의 결함엔 당일 재검증으로, 진단엔 원인 규명으로, 제안엔 첫 실증으로.", True, NAVY, 11)]])
footnote(s, RULE)
