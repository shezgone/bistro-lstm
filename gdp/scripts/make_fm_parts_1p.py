...
runs(X + 0.2, 3.8, W - 0.2, 2.55, [
    ...
    [("둘 다 공변량을 입력받습니다. 차이는 사전학습 때 공변량에게", False, INK, 9.5)],
    [("어떤 ", False, INK, 9.5), ("역할", True, INK, 9.5), ("을 주고 훈련했는가입니다.", False, INK, 9.5)],
    [("· Moirai(2024.03): “여러 시계열을 나란히 놓고 전부를 함께", False, INK, 9.5)],
    [("  예측하라”로 훈련 — 공변량은 ", False, INK, 9.5), ("함께 예측되는 이웃", True, INK, 9.5),
     (". 이웃을", False, INK, 9.5)],
    [("  참고해 타깃을 더 잘 맞히도록 채점받은 적은 없음", False, INK, 9.5)],
    [("· Chronos-2(2025.10): “저 시계열들을 참고해 이 시계열을", False, INK, 9.5)],
    [("  맞혀라”로 직접 훈련 — 공변량이 처음부터 ", False, INK, 9.5), ("힌트", True, INK, 9.5),
     (" 역할.", False, INK, 9.5)],
    [("  그래서 처음 보는 주가·환율도 힌트로 연결해 씀", False, INK, 9.5)],
    [("· 체급 차이 병기: small 변형(약 14M) vs C2 120M —", False, INK, 9.5)],
    [("  격차의 일부는 훈련 방식, 일부는 체급일 수 있음", False, INK, 9.5)],
], sp=1.8)
