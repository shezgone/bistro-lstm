# Graph Report - bistro-lstm  (2026-08-10)

## Corpus Check
- 136 files · ~99,529 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 1633 nodes · 2198 edges · 143 communities (114 shown, 29 thin omitted)
- Extraction: 97% EXTRACTED · 3% INFERRED · 0% AMBIGUOUS · INFERRED: 61 edges (avg confidence: 0.52)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `4f543b66`
...
- `BISTRO-LSTM` --references--> `kaleido>=0.2.1`  [EXTRACTED]
  README.md → requirements.txt
...
### Community 50 - "ablation_study.py"
Cohesion: 0.27
...
Cohesion: 0.12
Nodes (21): BISTRO-LSTM Core — Domain Classes ================================== bistro-xai의, BISTRO-LSTM Model — Attention-Augmented Stacked LSTM ===========================, LSTM Trainer — Walk-Forward CV + Optuna Tuning =================================, Optuna 하이퍼파라미터 튜닝.      Returns     -------     dict with best_params, best_valu, run_optuna_tuning(), set_seed(), create_sequences(), Preprocessing Utility for BISTRO-LSTM ===================================== 월별 매 (+13 more)
...
### Community 67 - "ImportanceAnalyzer"
Cohesion: 0.23
...
Cohesion: 0.15
Nodes (17): EarlyStopping, evaluate(), optuna_objective(), device, ndarray, 단일 fold 학습.      Returns     -------     dict with best_model_state, train_histo, Walk-forward cross-validation.      Parameters     ----------     splits : prepa, Optuna objective function.     Walk-forward CV의 평균 val RMSE를 최소화. (+9 more)

### Community 95 - "ZScoreNormalizer"
Cohesion: 0.27
Nodes (6): prepare_walk_forward_splits(), DataFrame, ndarray, Walk-forward CV용 데이터 분할.      Parameters     ----------     df : 전체 학습 데이터 (Peri, Per-variable z-score 정규화.     학습 세트 기준으로 mean/std 계산, 테스트 세트에 동일 적용., ZScoreNormalizer

### Community 96 - "AttentionLSTMForecaster"
Cohesion: 0.15
Nodes (18): compute_all_importance(), compute_gradient_importance(), compute_permutation_importance(), extract_attention_importance(), device, ndarray, Tensor, Feature Importance Methods for LSTM ==================================== 1. Vari (+10 more)
...
Cohesion: 0.09
Nodes (32): compare_forecasts(), compare_variable_rankings(), compute_metrics(), load_bistro_ablation(), load_bistro_results(), load_lstm_ablation(), load_lstm_results(), main() (+24 more)
...
- **Why does `AttentionLSTMForecaster` connect `AttentionLSTMForecaster` to `causal_narrative.py`, `LSTMConfig`, `phase_b_transformer.py`, `ablation_study.py`, `TorchSeq`, `ar1_forecast`, `Community 84`, `train_and_evaluate.py`?**
  _High betweenness centrality (0.012) - this node is a cross-community bridge._
- **Why does `LSTMConfig` connect `LSTMConfig` to `causal_narrative.py`, `ImportanceAnalyzer`, `comparison.py`, `ablation_study.py`, `ar1_forecast`, `train_and_evaluate.py`?**
  _High betweenness centrality (0.003) - this node is a cross-community bridge._
- **Why does `ImportanceAnalyzer` connect `ImportanceAnalyzer` to `ar1_forecast`, `comparison.py`?**
  _High betweenness centrality (0.003) - this node is a cross-community bridge._
