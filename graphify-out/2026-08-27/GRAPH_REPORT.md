# Graph Report - bistro-lstm  (2026-08-27)

## Corpus Check
- 152 files · ~121,162 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 1713 nodes · 2294 edges · 157 communities (125 shown, 32 thin omitted)
- Extraction: 97% EXTRACTED · 3% INFERRED · 0% AMBIGUOUS · INFERRED: 61 edges (avg confidence: 0.52)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `99f1f5a2`
...
Cohesion: 0.18
Nodes (15): load_stage1_screening(), Stage 1 전체 변수 스크리닝 결과 로딩., load_macro_panel(), Preprocessing Utility for BISTRO-LSTM ===================================== 월별 매, macro_panel.csv 로딩.      Parameters     ----------     csv_path : CSV 파일 경로, main(), device, ndarray (+7 more)
...
Cohesion: 0.21
Nodes (11): create_sequences(), prepare_walk_forward_splits(), DataFrame, ndarray, 슬라이딩 윈도우로 학습용 시퀀스 생성.      Parameters     ----------     data : (T, n_vars) 정규화된, Walk-forward CV용 데이터 분할.      Parameters     ----------     df : 전체 학습 데이터 (Peri, Per-variable z-score 정규화.     학습 세트 기준으로 mean/std 계산, 테스트 세트에 동일 적용., split_train_test() (+3 more)
...
Cohesion: 0.07
Nodes (38): compare_forecasts(), compare_variable_rankings(), compute_metrics(), load_bistro_ablation(), load_bistro_results(), load_lstm_ablation(), load_lstm_results(), main() (+30 more)
...
### Community 141 - "rolling_2025_torch.py"
Cohesion: 0.33
Nodes (5): set_seed(), Rolling 1-step-ahead forecast with LSTM and Transformer (same arch as report)., Rolling 1-step-ahead inference for origins 2025-04 to 2025-11., rolling_inference(), train_and_save()
...
- **Why does `AttentionLSTMForecaster` connect `AttentionLSTMForecaster` to `causal_narrative.py`, `LSTMConfig`, `ZScoreNormalizer`, `rolling_2025_torch.py`, `TorchSeq`, `ar1_forecast`, `Community 84`, `train_and_evaluate.py`, `train_and_evaluate.py`?**
  _High betweenness centrality (0.004) - this node is a cross-community bridge._
- **Why does `ZScoreNormalizer` connect `ZScoreNormalizer` to `AttentionLSTMForecaster`, `causal_narrative.py`, `LSTMConfig`, `rolling_2025_torch.py`, `ar1_forecast`, `train_and_evaluate.py`?**
...
- **What connects `실험 B: BISTRO LoRA 적응 — C2-LoRA(v1)와 동일 레시피, walk-forward 평가 (2026-08-26).  C2 대조`, `v1 레시피: 분기당 완성 경로 1개 (C2와 동일).`, `경로 → (past_target, past_feat, future_target). past는 CTX로 좌측 패딩/절단.` to the rest of the system?**
